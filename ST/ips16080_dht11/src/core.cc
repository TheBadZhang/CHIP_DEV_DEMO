...
void core(void) {
	...
	while(true) {
		...
		sprintf(str1, "wetness:%d.%d%%",DHT11_Data.humi_int,DHT11_Data.humi_deci);
		sprintf(str2, "temp:%d.%d", DHT11_Data.temp_int,DHT11_Data.temp_deci);
		DHT11_ReadData(&DHT11_Data);
		ST7735_WriteString(0, 0, str1, Font_7x10, ST7735_BLUE, ST7735_BLACK);
		ST7735_WriteString(0, 20, str2, Font_7x10, ST7735_RED, ST7735_BLACK);
		// loop();
		HAL_Delay(1000);
	}
