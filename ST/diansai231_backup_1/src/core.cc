...
*/
	...
	double t = (double)(signal_A.freq)*adc_freq/fft_size;
	double tt = 5.0*((int)((t+0.5)/5000+0.5));
	AD9833_WaveSeting(tt*1000,0,SIN_WAVE, 0);
	// 求相频
	// PowerPhase
	// for (int i = 0; i < fft_size; i++) {
...
void core(void) {
	...
	// AD9833_FreqSet(1000000); // 1MHz
	// AD9833_CtrlSet(1,3,0,0);
	// AD9833_CtrlSet(0,0,0,0);  // 输出正弦波
	// AD9833_CtrlSet(0,0,0,1);  // 输出三角波
	// AD9833_CtrlSet(0,0,1,0);  // 输出方波

	// AD9833_WaveSeting(30000.1,0,SIN_WAVE, 0);
	// AD
	while (true) {
		// flip(LED);
